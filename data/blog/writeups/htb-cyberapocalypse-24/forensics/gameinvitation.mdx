---
title: HTB - Cyber Apocalypse 2024 - Forensics - Game Invitation
date: '2024-03-14'
tags: ['forensics', 'htb', 'cyber-apocalypse', 'word-macro', 'js', 'hta', 'powershell']
draft: false
summary: Extracting a Word Macro, extracting an embedded hta/js code and finally extracting another powershell code that contains the flag.
---

## Challenge Description

![alt text](/static/writeups/htb-cyberapocalypse/image-12.png)

## Solution

Game Invitation was a hard forensics challenge that was pretty easy to solve and me and my team **AirOverflow** solved fairly quickly. The gist of the challenge was that we were provided with a simple Word Document with the extension of `docm`. Now, even though I'm not that well-versed with Forensics but I know that `docm` is the extension for files that contain macros.

> A macro is a small program that records the actions you take to complete a specific task.

I used `oletools`; specifically olevba to extract the macro from the word document:
...
Now, this code was highly obfuscated. One thing that I saw was `mailform.js` and `Environ("appdata") & "\Microsoft\Windows"`. And the `put` function in `vba`. So, I saw that the file was being stored in `Users\AppData\Roaming\Microsoft\Windows\mailform.js`. So, I simply commented out the `R66BpJMgxXBo2h.Run """" + IAiiymixt + """" + " vF8rdgMHKBrvCoCp0ulm"` call, as I didn't want to call the code, but store it. So I ran the macro.

However, after running the macro, the file wasn't created. Upon closer inspection of the code, I read this section carefully:
...
Now, what I did with this condition; simply removed it. After removing this condition, and re-running the macro. The file was created successfully.

> NOTE: You also need to delete the `END IF` at the bottom to completely nerf the if-statement.

![alt text](/static/writeups/htb-cyberapocalypse/image-13.png)

The file contents of this file were as follows:
...
Now, one thing I noticed off the bat was:
...
But, I still decided to clean the code a little bit, it became:

```js
var DASz = WScript.Arguments(0)
var Iwlh = lyEK()
Iwlh = JrvS(Iwlh)
Iwlh = xR68(DASz, Iwlh)
...
Now, here I did a simple thing; replaced `eval` with `WScript.stdout.writeline` function:

```diff
- eval(Iwlh);
+ WScript.Stdout.Writeline(Iwlh);
```

Now, running the script `.\mailform.js`, we get an error

![alt text](/static/writeups/htb-cyberapocalypse/image-14.png)

Now, after a simple google search, I found out that, I can run `js` code in windows with `cscript` command. So, I tried with that, but the error changed to `Subscript out of range`:

![alt text](/static/writeups/htb-cyberapocalypse/image-15.png)

At this point, I went back to the macro, and suddenly, the line that I commented out; popped up:

```vba
R66BpJMgxXBo2h.Run """" + IAiiymixt + """" + " vF8rdgMHKBrvCoCp0ulm"
```

Now, here `R66BpJMgxXBo2h` represents `WScript.Shell`, `IAiiymixt` represents the `js` code. The `vF8rdgMHKBrvCoCp0ulm` is an argument that was being passed. So, I passed the script this argument:

![alt text](/static/writeups/htb-cyberapocalypse/image-16.png)

Boom, we got another code. Let's analyze this code:
...
Now, I didn't really have to reverse this code, in sublime; I did a simple search for `flag` and got this:

```py
S47T.SETREQUESTHEADER("Cookie:","flag=SFRCe200bGQwY3NfNHIzX2czdHQxbmdfVHIxY2tpMTNyfQo=");
```

And a simple decoding of this:

![alt text](/static/writeups/htb-cyberapocalypse/image-17.png)
